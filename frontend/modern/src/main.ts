...
import { initUiScale } from './core/uiScale';
...
function init() {
  // The UI scale first: the frame is CSS-zoomed, so it decides the box the canvas gets when
  // initStore() sizes it (core/uiScale.ts, core/store.ts).
  initUiScale();
