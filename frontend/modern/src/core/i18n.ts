...
const dict = {
  en: {
    ...
    'if_overflow_hint': 'IF overflow (-12): the IF is saturated, so the device stops sending frames. Raise Ref until this clears.',
    'volume': 'Volume', 'squelch_short': 'Squelch', 'capture_bw': 'IQ capture bandwidth',
    'sdr_hint': 'click=tune · drag=tune/pan · ←→ tune · PgUp/PgDn BW · M mode · Space audio',
    'sdr_audio_tip': 'Enable/disable audio (Space)', 'agc_tip': 'Audio AGC',
    'sdr_listen_tip': 'Demod frequency (MHz)', 'sdr_center_tip': 'Wideband center (MHz)',
    'sdr_snap_tip': 'Set as active marker',
    ...
    'tip_atten': 'Channel attenuation: Auto is derived from the reference level; a manual value fixes it (the reference level still sets the total gain, so the displayed level barely moves).', 'tip_peakthr': 'Peak threshold (dBm); Auto = global peak - 50dB, manual edit keeps your value',
  ...
  zh: {
    ...
    'if_overflow_hint': '中频饱和 (-12)：中频已过载，设备停止出帧。请上调 Ref 直到该告警消失。',
    'volume': '音量', 'squelch_short': '静噪', 'capture_bw': 'IQ 捕获带宽',
    'sdr_hint': '单击=调谐 · 拖动=调谐/平移 · ←→ 调谐 · PgUp/PgDn 带宽 · M 解调 · Space 声音',
    'sdr_audio_tip': '启用/关闭声音（Space）', 'agc_tip': '音频 AGC',
    'sdr_listen_tip': '解调频率（MHz）', 'sdr_center_tip': '宽带中心（MHz）',
    ...
    'tip_atten': '通道衰减：自动=由参考电平决定；手动=固定衰减值（总增益仍由参考电平决定，因此显示电平基本不变）。', 'tip_peakthr': '峰值阈值(dBm); 自动=全局峰值-50dB, 手动编辑保留您的值',
...
Object.assign(dict.en, {
  ...
  'preamp_manual_atten': 'Ignored while Atten is manual: the device decides the preamplifier itself',
...
Object.assign(dict.zh, {
  ...
  'preamp_manual_atten': '衰减为手动时该项无效：前置放大器由设备自行决定',
...
Object.assign(dict.zh, {'tip_trg_capture': '单次捕获：启用电平触发，下一次穿越门限时捕获一次，随后自动回到自由运行'});
// RTA trigger panel
