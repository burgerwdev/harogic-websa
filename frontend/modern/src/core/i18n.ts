// i18n en/zh dictionary. Every key is declared exactly once per language, so the two
// languages can be compared mechanically (see __tests__/i18n.test.ts).
//
// History: this used to be a base literal plus thirteen additive Object.assign blocks,
// which left `keyof typeof dict.en` covering only the base literal and made key parity
// impossible to check. Merged into one declaration in 2026-09; values are unchanged
// (later assignments still win, as they did at runtime).
const dict = {
  en: {
    'ref_level': 'Ref Level:',
    'scale': 'Scale:',
    ...
    'volume': 'Volume',
    'squelch_short': 'Squelch',
    'capture_bw': 'IQ capture bandwidth',
    'sdr_hint': 'click=tune · drag=tune/pan · ←→ tune · PgUp/PgDn BW · M mode · Space audio',
    'sdr_audio_tip': 'Enable/disable audio (Space)',
    'agc_tip': 'Audio AGC',
    'sdr_listen_tip': 'Demod frequency (MHz)',
    'sdr_center_tip': 'Wideband center (MHz)',
    ...
    'off': 'Off',
    'if_gain': 'IF Gain',
    'gain_strategy': 'Gain St',
    'low_noise': 'Low Noise',
    'high_linearity': 'High Linearity',
    'trace': 'Trace',
    'mode': 'Mode',
    ...
    'persist': 'Persist',
    'grain': 'Grain',
    'span_down': 'Narrower span',
    'span_up': 'Wider span',
    'ref_down': 'Lower ref level',
    'ref_up': 'Raise ref level',
    'span_full': 'Full bandwidth (50.8 MHz)',
    'span_full_short': 'Full',
    'smooth': 'Smooth',
    'off_short': 'Off',
    'cal_wnd': 'Cal Wnd',
    'auto_rbw': 'Auto (RBW)',
    'normalize': 'Normalize',
    'reset_norm': 'Reset Norm',
    'clear': 'Clear',
    'marker': 'Marker',
    'active': 'Active',
    'tracking': 'Tracking',
    ...
    'bw': 'BW',
    'manual': 'Manual',
    'auto_span2000': 'Auto (Span/2000)',
    'window': 'Window',
    'bypass_10x': 'Bypass (10×RBW)',
    'eq_rbw': '= RBW',
    'tenth_rbw': '= 0.1×RBW',
    'sweep': 'Sweep',
    'points': 'Points',
    ...
    'status_nolock': 'NoLock',
    'gnss_detail': 'GNSS Detail',
    'gnss_lock': 'Lock',
    'gnss_sats': 'Satellites',
    'gnss_docxo': 'DOCXO',
    'gnss_time': 'Time',
    'gnss_close': 'Close',
    ...
    'gnss_antenna': 'Antenna',
    'gnss_docxo_mode': 'DOCXO Mode',
    'gnss_ext_ant': 'External',
    'gnss_int_ant': 'Internal',
    'gnss_utc_time': 'GNSS UTC Time',
    'gnss_local_time': 'System Time',
    'gnss_docxo_lock': 'Disciplined',
    'gnss_docxo_hold': 'Hold',
    ...
    'mk_marker': 'Marker',
    'mk_mode': 'Mode',
    'mk_freq': 'Frequency / ΔFreq',
    'mk_amp': 'Amplitude / ΔPower',
    'mk_ref': 'Reference',
    'm3db_bw': '3dB BW',
    'm3db_center': 'Center',
    'm3db_q': 'Q',
    'harm_table_title': 'Harmonics',
    'pnm_title': 'Phase Noise',
    'tip_connect': 'Connect to device',
    'tip_norm': 'Store current trace as reference and enable normalize',
    'tip_atten': 'Channel attenuation: Auto is derived from the reference level; a manual value fixes it (the reference level still sets the total gain, so the displayed level barely moves).',
    'tip_peakthr': 'Peak threshold (dBm); Auto = global peak - 50dB, manual edit keeps your value',
    ...
    'avg': 'Avg',
    'export_csv': 'CSV',
    'avg_opt_16': '16 (default)',
    'detector': 'Detector',
    'tip_detector': 'Trace detector (SWP only). Auto Sample follows the signal (default). Sample / Pos Peak / Neg Peak / RMS are fixed detectors. Auto Peak is not offered in the UI: measured on SAN-90 it loses a steady CW carrier, so use it only via the API for pulsed signals.',
    'norm_cleared': 'Normalization cleared (measurement settings changed)',
    ...
    'tip_version': 'Frontend version',
    'kp_drag': 'Drag to move the keypad',
    'kp_comma': 'Comma: separates the threshold values',
    'tip_keypad': 'Virtual keypad: click a numeric field to enter the value with the mouse',
    'kp_ok': 'Confirm',
    'kp_clear': 'Clear',
    'kp_back': 'Delete',
    'kp_minus': 'Negative sign',
    'bad_data': 'NO VALID DATA (input overloaded?)',
    'wf_range': 'WF Range',
    'wf_auto': 'Auto',
    'wf_fixed': 'Fixed',
    'tip_wf_range': 'Waterfall colour range. Auto maps each frame relative to its own noise floor (the texture is always visible); Fixed maps an absolute dBm window so colours are quantitative',
    'tip_wf_lo': 'Fixed range lower level: everything at or below this is drawn as background',
    'tip_wf_hi': 'Fixed range upper level: everything at or above this is drawn as full-scale',
    ...
    'chan_obw': 'OBW',
    'chan_acp': 'ACP',
    'chan_power': 'Channel power',
    'chan_acp_l': 'ACP lower',
    'chan_acp_u': 'ACP upper',
  ...
  zh: {
    ...
    'sdr_audio_tip': '启用/关闭声音（Space）',
    'agc_tip': '音频 AGC',
    'sdr_listen_tip': '解调频率（MHz）',
    'sdr_center_tip': '宽带中心（MHz）',
    'stop': '终止',
    'set': '设置',
    'full_span': '全频段',
    'span_step': '扫宽步进',
    ...
    'bw': '带宽',
    'manual': '手动',
    'auto_span2000': '自动(扫宽/2000)',
    'window': '窗口',
    'bypass_10x': '旁路 (10×RBW)',
    'eq_rbw': '= RBW',
    'tenth_rbw': '= 0.1×RBW',
    'sweep': '扫描',
    'points': '点数',
    ...
    'gnss_altitude': '海拔',
    'gnss_antenna': '天线',
    'gnss_docxo_mode': 'DOCXO 模式',
    'gnss_ext_ant': '外部',
    'gnss_int_ant': '内部',
    'gnss_utc_time': 'GNSS UTC 时间',
    'gnss_local_time': '系统时间',
    'gnss_docxo_lock': '驯服',
    'gnss_docxo_hold': '跟踪',
    ...
    'pnm_title': '相位噪声',
    'tip_connect': '连接设备',
    'tip_norm': '将当前迹线存为参考并启用归一化',
    'tip_atten': '通道衰减：自动=由参考电平决定；手动=固定衰减值（总增益仍由参考电平决定，因此显示电平基本不变）。',
    'tip_peakthr': '峰值阈值(dBm); 自动=全局峰值-50dB, 手动编辑保留您的值',
    ...
    'avg_opt_16': '16（默认）',
    'detector': '检波',
    'tip_detector': '迹线检波器（仅普通频谱 SWP）。自动取样跟随信号（默认）；随机/正峰值/负峰值/RMS 为固定检波方式；自动峰值不在界面提供：在 SAN-90 上实测会丢失稳定 CW 载波，仅在脉冲场景下通过 API 使用。',
    'norm_cleared': '归一化已清除（测量参数已变化）',
    ...
    'kp_comma': '逗号：分隔各个门限值',
    'kp_drag': '拖动可移动小键盘',
    'tip_keypad': '虚拟小键盘：点击任意数字输入框即可用鼠标输入数值',
    'kp_ok': '确定',
    'kp_clear': '清除',
    'kp_back': '退格',
    'kp_minus': '负号',
    'bad_data': '无有效数据（输入是否过载？）',
    'wf_range': '瀑布范围',
    'wf_auto': '自动',
    'wf_fixed': '固定',
    'tip_wf_range': '瀑布色标范围。自动：按每帧自身噪底相对映射（底噪纹理始终可见）；固定：按绝对 dBm 区间映射，颜色可直接读电平',
    'tip_wf_lo': '固定范围下限：低于该电平画为背景',
    'tip_wf_hi': '固定范围上限：高于该电平画为满量程',
