...
5. background *safety* correction is separate, always armed, rate-limited and PROTECTIVE-only (IF
   ...
   control the user just used. A settings change (span/centre/RBW/window/decimation) is the one
   exception that may move a level the loop itself placed: it arms exactly ONE re-fit for the new
   geometry (disarmed by its first decision, so it cannot become a tracking loop), while a level the
