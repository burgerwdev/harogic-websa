...
| **Auto Scale one-shot** | Ref "Auto" became a one-shot action (`AUTO_SCALE`) with a busy/result indication; the IF-overflow safety ranger is always armed (manual Atten no longer disables it) and a trace that left the window is fitted automatically; the peak-list threshold is fitted once per geometry from a robust estimate. Measured on the bench: click -> landing 0.11-0.12 s (was 1.86 s), second press `ok` with no reconfiguration. SDR settings (tuning, capture bandwidth, demodulator, IF bandwidth, de-emphasis, volume, squelch, AGC) became persisted preferences that are re-applied on entry, with the swept-centre hand-off moved to the explicit Shift+click gesture (it used to re-derive everything on every entry). The safety ranger was then made direction-aware (protective only: IF overflow and gross clipping; raising Ref is never undone) and the Auto message moved into the canvas status stack. SDR was moved onto the same `AUTO_SCALE` path (it used to re-implement the rule in TypeScript and was invisible to STATUS): the backend fits from the panadapter trace and the client applies the target to its display scale | `make ci`, `make e2e-fake`, `make hw-test` |
