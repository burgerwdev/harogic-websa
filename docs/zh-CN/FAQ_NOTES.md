...
- 拟合结果通过 `auto_ref.result` 说明：`applied`（含 `target`）、`ok`（已经放好）、`no_signal`（峰值比噪底高
  不足 15 dB **且**迹线在窗口内）、`no_data`（上次重配后还没有迹线）。有变更排队或正在稳定时按钮会发光，
  提示栏说明结果。
- **安全量程始终运行**，与 Atten 设置、是否按过 Auto 都无关：IF 溢出（-12）每秒抬 5 dB；整条迹线离开
  显示窗口（峰高于上沿或噪声底低于下沿）时拟合一次并限速。这正是过去手动 Atten 会关掉的东西——过载
  保护不会再被误关。因此：**手动**设定的 Ref 若让整条迹线离开窗口，会被纠正一次（替代方案是空白画面且
  无任何解释）；把电平设到迹线能放下的位置即重新掌握控制，点一次 Auto 会让两者重新一致。
