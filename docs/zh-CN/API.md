...
| `auto_ref` | obj | 参考电平放置状态：`last_peak`/`last_noise_floor`（最新迹线）、`target`（上次拟合应用的电平）、`result`（`idle`/`applied`/`ok`/`no_signal`/`no_data`/`out_of_window`/`overflow`）、`pending`（排队中的电平）、`adjusting`（有变更排队或正在稳定） |
...
| `AUTO_SCALE` | `range_db?` | 按最新迹线一次性放置参考电平：噪声底落在 `range_db` 高窗口底部稍上。放置已合理时不做任何事（不重配器件）。结果通过 `auto_ref.result` 报告；SDR 模式不可用（该模式的显示刻度由客户端负责） |
