...
| `auto_ref` | obj | 参考电平放置状态：`last_peak`/`last_noise_floor`（最新迹线）、`target`（上次拟合应用的电平）、`result`（`idle`/`applied`/`ok`/`no_signal`/`no_data`/`out_of_window`/`overflow`）、`seq`（每次决策自增）、`pending`（排队中的电平）、`adjusting`（有变更排队或正在稳定） |
...
| `AUTO_SCALE` | `range_db?`, `current_ref?` | 按最新迹线一次性放置参考电平：噪声底落在 `range_db` 高窗口底部稍上。`current_ref` 是用户当前看到的电平——SDR 的显示刻度由客户端负责，判据必须以它为准。放置已合理时不做任何事（不重配器件）。结果通过 `auto_ref.result`/`target`/`seq` 报告 |
